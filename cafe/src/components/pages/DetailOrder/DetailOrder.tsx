          ...
          <div className={'mt-10'}>
            ...
            <div className={'grid grid-cols-3 gap-5 mt-5'}>
              {data?.cart?.map((item: ICart) => (
                <div key={item.menuItem?.id} className={'bg-gray-200 flex items-center p-4 rounded-2xl gap-3'}>
                  <img className={'object-cover rounded-xl w-15 h-15'} src={item?.menuItem?.image_url} alt={item?.menuItem?.name} />
                  <div>
                    <p>
